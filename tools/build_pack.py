"""Build immutable release assets from an existing game directory (no saves)."""
import argparse
import hashlib
...
def build(source, output, pack_id, name, version, minecraft, forge, installed_version, java, repository):
    source, output = Path(source).resolve(), Path(output).resolve()
    if output.is_relative_to(source):
        raise ValueError('Каталог выпуска должен находиться вне исходной сборки')
    if output.exists() and any(output.iterdir()):
        raise ValueError('Для нового выпуска нужен пустой каталог')
    ...
    base = f'https://github.com/{repository}/releases/download/{quote(pack_id + "-" + version, safe="")}/'
    for file in sorted(source.rglob('*')):
        ...
        manifest['files'].append(dict(path=relative, size=file.stat().st_size, sha256=digest, url=base + asset,
...
if __name__ == '__main__':
    ...
    result = build(args.source, args.output, args.id, args.name, args.version, args.minecraft, args.forge,
                   args.installed_version, args.java, args.repository)
    print(f"Готово: {len(result['files'])} файлов. Манифест: {Path(args.output) / 'manifest.json'}")
